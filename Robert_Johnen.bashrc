...
export PS1='${MAGENTA}Ende  Job: - \d | \t Uhr -\n$([[ $? -eq 0 ]] && echo "${CGOOD} ✓ " || echo "${CERROR} ✗ Exitcode: $? ")${CRESET}${WHITE} | Mails: $(grep -c "^Return-Path: " /var/spool/mail/${USER} 2>/dev/null || echo -n "0") |${WHITE} lfd. bg-Jobs: \j | lfd. screen: $(screen -ls 2>/dev/null | grep Detached | wc -l) | lfd. tmux: $(tmux ls 2>/dev/null | wc -l)\n${RED}[${BLUE}\W${RED}]${CYAN} | Links: $(ls -al | grep -e ^l | wc -l) | Verzeichnisse: $(ls -al | grep -e ^d | wc -l) | Dateien: $(ls -al | grep -e ^- | wc -l) | FIFOs: $(ls -al | grep -e ^p | wc -l)\n$([[ $UID -eq 0 ]] && echo ${RED} || echo ${YELLOW})\u@\h:${BLUE}\w${CRESET} \$> '
